...
#
# Links across three libraries: Crankstart, TestTool, and BareMetalHAL.
# Source discovery/linking mirrors arduino-cli's own approach: every
# source file under a library's src/ is compiled, archived into a static
# library, and the linker pulls in only what main() actually references.
...
CFLAGS=(-std=gnu++11 -Wall -Wextra -Os -DNO_ARDUINO -DHAL_AVR -DDEBUG -DF_CPU=16000000UL -mmcu=atmega2560 -I "$DIR/../../src" -I "$BAREMETALHAL_SRC" -I "$TESTTOOL_SRC")

mkdir -p "$OBJ_DIR"

# build_archive <name> <src-root>
# Compiles every *.cpp found (recursively) under <src-root> and archives
# the resulting objects into $BUILD_DIR/lib<name>.a.
build_archive() {
  local name="$1"
  local src_root="$2"
...
build_archive crankstart "$DIR/../../src" >/dev/null
...
"$AVRGXX" "${CFLAGS[@]}" \
  ...
  -L "$BUILD_DIR" -lcrankstart -ltesttool -lbaremetalhal
